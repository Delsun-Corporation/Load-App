...
class StartWorkoutCardioVC: UIViewController {
    
    //MARK:- Variables
    
    lazy var mainView: StartWorkoutCardioView = { [unowned self] in
        return self.view as! StartWorkoutCardioView
        }()
    
    lazy var mainModelView: StartWorkoutCardioViewModel = { [weak self] in
        return StartWorkoutCardioViewModel(theController: self)
    }()
    ...
    override func viewDidLoad() {
        ...
        //Swimming only have pace
        if self.mainModelView.activityName.lowercased() == "Swimming".lowercased(){
            self.isSpeed = false
        }
...
//            self.mainView.lblCount.text = self.setOneDigitWithFloor(value: (CGFloat(lapCoveredDistance)/1000))
    ...
    override func viewWillAppear(_ animated: Bool) {
        NotificationCenter.default.addObserver(self, selector: #selector(updateTotalDistance(notification:)), name: Notification.Name(NOTIFICATION_CENTER_LIST.LOAD_CARDIO_UPDATED_TOTAL_DISTANCE.rawValue), object: nil)
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        NotificationCenter.default.removeObserver(self, name: Notification.Name(NOTIFICATION_CENTER_LIST.LOAD_CARDIO_UPDATED_TOTAL_DISTANCE.rawValue), object: nil)
    ...
    @IBAction func btnNextClicked(_ sender: Any) {
        ...
        if self.mainModelView.exerciseArray[self.mainModelView.index].isCompleted == false || self.mainModelView.exerciseArray[self.mainModelView.index].isCompleted == nil{
            ...
            let valueAddigForRest = date.convertDateFormater().addingTimeInterval(TimeInterval(self.mainModelView.getSeconds(data: self.mainModelView.exerciseArray[self.mainModelView.index].rest)))
            ...
            if self.mainModelView.index == self.mainModelView.exerciseArray.count - 1{
                ...
                if self.mainModelView.exerciseArray[self.mainModelView.index].rest == nil ||  self.mainModelView.exerciseArray[self.mainModelView.index].rest == ""{
        ...
        if sender.isSelected {
            ...
            UIView.animate(withDuration: 0.5,
                           ...
                           animations: {
                            ...
                            //                    self.mainView.vwCounterAndText.isHidden = true
            ...
            UIView.animate(withDuration: 0.5,
                           ...
                           animations: {
                            ...
                            //                    self.mainView.vwCounterAndText.isHidden = false
...
//                            routeObjects[0].isPauseAfterAllLapCompleted = self.mainModelView.isPaused
        ...
        //MARK:- If write here then playpause icon change
        sender.isSelected = !sender.isSelected
...
//                }
        ...
        if !self.mainModelView.isPaused {
            ...
            //            if self.mainModelView.exerciseArray[0].duration == "" || self.mainModelView.exerciseArray[0].duration == nil{
            ...
            if self.mainModelView.activityName.lowercased() == "Run (Outdoor)".lowercased() || self.mainModelView.activityName.lowercased() == "Cycling (Outdoor)".lowercased() {
                ...
                self.reCallMainCountingStep()
                if let valueForIndoor = Defaults.value(forKey: "trainingLog Indoor \(self.mainModelView.trainingLogId)") as? Double{
                    
                    print("ValueForIndoor:\(valueForIndoor)")
                    
                    Defaults.removeObject(forKey: "trainingLog Indoor \(self.mainModelView.trainingLogId)")
...
//                self.dateClickOnPauseButton = Date()
            ...
            if self.mainModelView.activityName.lowercased() == "Run (Outdoor)".lowercased() || self.mainModelView.activityName.lowercased() == "Cycling (Outdoor)".lowercased() {
                ...
                if self.mainModelView.isRest == false && self.mainModelView.isDuration == false{
                    Defaults.set(self.distanceOfLapForIndoor, forKey: "trainingLog Indoor \(self.mainModelView.trainingLogId)")
                    Defaults.synchronize()
                }
            }
...
//        }
    ...
    func endWorkoutAlert(){
        ...
        let cancelAction = UIAlertAction(title: getCommonString(key: "No_key"), style: UIAlertAction.Style.cancel) { [self] (result : UIAlertAction) -> Void in
            ...
            //TODO: - Comments this line and check for Out door scenario
//            self.mainModelView.exerciseArray[0].startTime = convertToStringForStartTime
        ...
        //                }
            ...
            if self.mainModelView.exerciseArray[self.mainModelView.index].addedRestTime == "" {
            ...
            */
            ...
            //Set this condition before manage End click for Duration/Rest paramter
            /*
            let activityName = self.mainModelView.activityName.lowercased()
            ...
            //            if self.mainModelView.exerciseArray[self.mainModelView.index].duration == "" || self.mainModelView.exerciseArray[self.mainModelView.index].duration == nil{
            //
            //                let date = Date().toString(dateFormat: "yyyy-MM-dd HH:mm:ss")
            //
            //                let valueAddigForRest = date.convertDateFormater().addingTimeInterval(TimeInterval(self.mainModelView.getSeconds(data: self.mainModelView.exerciseArray[self.mainModelView.index].rest)))
...
//        self.mainView.countdownTimer.isSolidLine = true
        ...
        self.mainModelView.apiCallRepeatWorkout(programId: self.mainModelView.trainingLogId, isEndWorkout: false, progress: true)
    ...
    func checkChangeAbleParameter(){
        ...
        case "Cycling (Indoor)".lowercased():
            ...
            if self.nextVariableCount == 0{
                ...
                isAvgSpeedOrPace = false
                
                if self.mainModelView.exerciseArray.count > 0{
                    if self.mainModelView.exerciseArray[0].watt != nil{
                        self.mainView.lblChangableParameter.text = "Watt (W)"
                        self.mainView.lblChangeableParameterValue.text = "-"
//                        "\(self.mainModelView.exerciseArray[self.mainModelView.index].watt ?? 0)"
                    }else{
                        self.mainView.lblChangableParameter.text = "RPM"
                        self.mainView.lblChangeableParameterValue.text = "-"
//                        "\(self.mainModelView.exerciseArray[self.mainModelView.index].rpm ?? 0)"
                    }
                    self.nextVariableCount = 0
                }
...
//                        "\(self.mainModelView.exerciseArray[self.mainModelView.index].rpm ?? 0)"
        ...
        case "Swimming".lowercased():
            ...
            //            if self.nextVariableCount == 0{
            self.isSpeed = false
            self.mainView.lblChangableParameter.text = "Avg. Pace (min/100m)"
            self.mainView.lblChangeableParameterValue.text = lastPace
            //                self.nextVariableCount = 0
            //            }
        ...
        case "Others".lowercased():
            ...
            if self.nextVariableCount == 0{
                ...
                self.mainView.lblChangeableParameterValue.text = lastPace
                self.nextVariableCount += 1
            }else if self.nextVariableCount == 2{
...
//                        "\(self.mainModelView.exerciseArray[self.mainModelView.index].rpm ?? 0)"
    ...
    func changeLabelName(){
        ...
        self.mainView.imgDropDown.isHidden = false
        ...
        case "Swimming".lowercased():
            self.mainView.imgDropDown.isHidden = true
            self.mainView.lblChangableParameter.text = "Avg. Pace (min/100m)"
...
//MARK: - Pedometer data
extension StartWorkoutCardioVC{
    ...
    private func startCountingSteps(fromDate:Date) {
         pedometer.startUpdates(from: fromDate) {
            ...
            DispatchQueue.main.async {
                ...
                print("STARTWORKOUT  TOTAL Distance :\(pedometerData.distance?.floatValue)")
                ...
                if self?.mainModelView.exerciseArray[0].duration == nil || self?.mainModelView.exerciseArray[0].duration == ""{
                    ...
                    if self?.mainModelView.isDuration == false
                    {
                        ...
                        print("Index startWorkOut:\(self?.mainModelView.index)")
...
//                            self?.updateStepsCountLabelUsingForRepeat(startDate: dateRepeat ?? Date())
                        }
                    }
            ...
            DispatchQueue.main.async {
                ...
                if self?.mainModelView.isDuration == false
                {
                    ...
                    if self?.mainModelView.exerciseArray[self?.mainModelView.index ?? 0].isCompleted == true && self?.mainModelView.exerciseArray[self?.mainModelView.index ?? 0].isCompletedRest == true{
                        ...
                        print("BEFORE RETURN INDEX: \(self?.mainModelView.index)")
                        
//                        if self?.mainModelView.index == (self?.mainModelView.exerciseArray.count ?? 0) - 1{
                            return
//                        } else {
//                            self?.distanceOfLapForIndoor = (self?.mainModelView.exerciseArray[self?.mainModelView.index ?? 0].distance ?? 0.0) * 1000
//                            self?.mainModelView.playNext()
//                            return
//                        }
                    }
...
//                    self?.findingAvgSpeed()
                    ...
                    let totalDistanceValue = (self?.mainModelView.exerciseArray[self?.mainModelView.index ?? 0].distance ?? 0.0) * 1000.0
...
extension StartWorkoutCardioVC: updateLatLongDelegate{
...
//                    makeToast(strMessage: "++++++++++++++ CHANGE INDEX HEREREE +++++++++++++++")
                ...
                if self.mainModelView.exerciseArray.count > 0{
                    if self.mainModelView.exerciseArray[0].duration == nil || self.mainModelView.exerciseArray[0].duration == ""{
                        if (self.mainModelView.exerciseArray[self.mainModelView.index].isCompleted == nil || self.mainModelView.exerciseArray[self.mainModelView.index].isCompleted == false) && (self.mainModelView.exerciseArray[self.mainModelView.index].isCompletedRest == nil || self.mainModelView.exerciseArray[self.mainModelView.index].isCompletedRest == false){
                            ...
                            print("StartWorkout lapCoveredDistance = \(lapCoveredDistance)")
...
//MARK: - take screenshot for LogPreview screen
...
extension StartWorkoutCardioVC{
    ...
    open func takeScreenshot() -> UIImage? {
        ...
        return UIGraphicsImageRenderer(size: self.view.bounds.size).image { _ in
            self.view.drawHierarchy(in: CGRect(x: 0, y: -45, width: self.view.bounds.width, height: self.view.bounds.height), afterScreenUpdates: true)
        }
    }
...
/*
//MARK: - Redirection according activity
...
extension StartWorkoutCardioVC: dismissConfirmationPageDelegate{
...
extension StartWorkoutCardioVC: delegateDismissRPESelection{
