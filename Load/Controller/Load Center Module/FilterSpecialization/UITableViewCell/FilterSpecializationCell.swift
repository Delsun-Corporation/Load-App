...
protocol FilterSpecializationSelectDelegate: class {
    func FilterSpecializationSelectDidFinish(section:Int, index:Int)
}

class FilterSpecializationCell: UITableViewCell {
    ...
    var index:Int = 0
    weak var delegate:FilterSpecializationSelectDelegate?
    ...
    @IBAction func btnSelectClicked(_ sender: Any) {
        ...
        self.delegate?.FilterSpecializationSelectDidFinish(section: self.tag, index: self.index)
    }    
}
