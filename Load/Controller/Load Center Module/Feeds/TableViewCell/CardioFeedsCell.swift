...
class CardioFeedsCell: UITableViewCell {
    ...
    @IBOutlet weak var btnLComment: UIButton!
    ...
    func setupFont() {
        ...
        self.lblLikes.font = themeFont(size: 9, fontname: .ProximaNovaRegular)
        self.lblComments.font = themeFont(size: 9, fontname: .ProximaNovaRegular)
        ...
        self.lblLikes.setColor(color: .appthemeBlackColor)
        self.lblComments.setColor(color: .appthemeBlackColor)
...
//        self.setupFont()
        ...
        if model.likedDetail != nil {
            self.lblLikesCount.text = self.getLikeStatus(isLiked: (model.likedDetail?.isLiked)!, count: model.likedDetail?.userIds?.count ?? 0)
            self.isLike(isLike: (model.likedDetail?.isLiked)!)
        }
        else {
            self.lblLikesCount.text = "Be the first to like!"
        }
    ...
    func isLike(isLike:Bool) {
        if isLike {
            self.btnLike.setImage(UIImage(named: "ic_like_thumsup_pink_color"), for: .normal)
        }
        else {
    ...
    func getLikeStatus(isLiked:Bool, count:Int) -> String {
        ...
        if countNew <= 0 && isLiked {
            ...
            str = "Be the first to like!"
        }
        else if isLiked {
            str = "You and \(countNew) athletes like this"
        }
        else {
            str = "\(countNew) athletes like this"
        }
        return str
    }
...
//            numberForTruncatedCell: 0,
    ...
    @IBAction func btnCommentClicked(_ sender: Any) {
        self.delegate?.FeedsCommentActionDidFinish(tag: self.tag)
    }

    @IBAction func btnShareClicked(_ sender: Any) {
       self.delegate?.FeedsShareActionDidFinish(tag: self.tag)
    }
...
//        self.setupFont()
        ...
        imgProfile.backgroundColor = .appthemePinkColor
        let camera: GMSCameraPosition = GMSCameraPosition.camera(withLatitude: 48.857165, longitude: 2.354613, zoom: 8.0)
        viewMap.camera = camera
        self.lblName.text = "Christina"
//        self.lblDate.text = convertDateFormater((Date()), dateFormat: "MMM dd, yyyy 'at' HH:mm a")
        self.lbltype.text = "Cool Down Cycle"
        self.lblFieldOneTitle.text = "Completed Vol. (kg)"
        self.lblFieldOne.text = "1222"
        self.lblFieldTwoTitle.text = "Total Exercise"
        self.lblFieldTwo.text = "9"
...
//        else {
            self.lblLikesCount.text = "Be the first to like!"
//        }
        self.lblCommentCount.text = "1 Comments"
    }
    
}

extension CardioFeedsCell: BPDelegate {
