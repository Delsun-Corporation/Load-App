...
protocol EquipmentSelectDelegate: class {
    func EquipmentSelectDidFinish(section:Int, index:Int)
}

class EquipmentSelectionCell: UITableViewCell {
    ...
    var index:Int = 0
    weak var delegate:EquipmentSelectDelegate?
    ...
    @IBAction func btnSelectClicked(_ sender: Any) {        
        ...
        self.delegate?.EquipmentSelectDidFinish(section: self.tag, index: self.index)
    }    
}
