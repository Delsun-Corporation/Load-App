//
//  LibraryExerciseListViewModel.swift
//  Load
//
...
class LibraryExerciseListViewModel {
    //MARK:- Variables
    fileprivate weak var theController:LibraryExerciseListVC!
    
    weak var delegate: AddLibraryExerciseDelegate?
    var listArray: LibraryListModelClass?
    var bodyParts: Category?
    var index:Int  = 0
    ...
    init(theController:LibraryExerciseListVC) {
    ...
    func setupUI() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(searchRecords(notification:)), name: Notification.Name(NOTIFICATION_CENTER_LIST.LIBRARY_EXERCISE_LIST_SEARCH_NOTIFICATION.rawValue), object: nil)
    }
    ...
    @objc func searchRecords(notification: Notification) {
        if let text = notification.userInfo?["data"] as? String {
            ...
            if  SELECTED_EXERCISE_LIBRARY_TAB == (self.bodyParts?.id?.stringValue)! {
                if text == "" {
                    self.apiCallLibraryList(status: (self.bodyParts?.code)!, id: (self.bodyParts?.id?.stringValue)!)
                }
                else {
                    self.apiCallSearchLibraryList(searchText: text, status: (self.bodyParts?.code)!, id: (self.bodyParts?.id?.stringValue)!, isLoading: false)
                }
            }
...
//            }
    ...
    @objc func refresh(sender:AnyObject) {
        self.apiCallLibraryList(status: (self.bodyParts?.code)!, id: (self.bodyParts?.id?.stringValue)!, isLoading: false)
    }
...
//                    if status.lowercased() != "FAVORITE".lowercased() {
                        ...
                        model?.index = NSNumber(integerLiteral: self.index)
                        self.delegate?.AddLibraryExerciseDidFinish(listArray: model!)
...
//                        self.delegate?.AddLibraryExerciseDidFinish(listArray: model!, favoriteArray: self.listFavoriteArray!)
...
//                        self.listArray = listUpdated
                    ...
                    /*
                        ...
                        self.delegate?.AddLibraryExerciseFavoriteDidFininsh(favoriteArray: model!)
                        ...
                        for data in arraySubNameSort {
                            ...
                            let modelFilter = filter?.sorted(by: { (model1, model2) -> Bool in
                                return model1.exerciseName!.compare(model2.exerciseName ?? "") == ComparisonResult.orderedAscending
                            })
...
//                        self.delegate?.AddLibraryExerciseDidFinish(listArray: self.listArray!, favoriteArray: model!)

                    }*/
                    
                    let view = (self.theController.view as? LibraryExerciseListView)
...
//                    if status.lowercased() != "FAVORITE".lowercased() {
                        ...
                        print("FilterList Arrat Cout :\(self.filterListArray?.count)")
                        print("FilterList list Cout :\(self.filterListArray?.list?.count)")
                    /*
                    }
                    else {
                        //                        let model = LibraryFavoriteListModelClass(JSON: data.dictionaryObject!)
